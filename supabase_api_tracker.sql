...
CREATE POLICY "Enable read access for all users" ON public.tuc_usage_stats
    FOR SELECT USING (true);

-- 由於 increment_gemini_usage 是 SECURITY DEFINER，它會以建立者權限執行 INSERT/UPDATE
-- 因此不需要額外開放 table 的直接寫入權限給 anon，這更安全。
