...
import { Ban, ShieldAlert, X, PenTool, BookOpen, Eye, Trash2, Download, Lock, Database, CloudUpload, Sparkles, Zap, Loader2, Check, Minimize2, Maximize2, Repeat, Info, Clock, RefreshCw } from 'lucide-react';
...
function App() {
  const [data, setData] = useState<FormState>(() => {
    ...
    const savedCache = localStorage.getItem('tuc_bilingual_cache');
    ...
    if (savedCache) {
      initialState.bilingualCache = JSON.parse(savedCache);
    }
    return initialState;
  });
  ...
  // V30.3: 持久化雙語快取
  useEffect(() => {
    localStorage.setItem('tuc_bilingual_cache', JSON.stringify(data.bilingualCache));
  }, [data.bilingualCache]);
  ...
  useEffect(() => {
    ...
    const targetLang = data.language === 'th-TH' ? data.primaryLanguage : data.language;
    if (prevLangRef.current !== targetLang) {
      syncFormHintsLanguage(targetLang);
      prevLangRef.current = targetLang;
    }
  }, [data.language, data.primaryLanguage]);
  ...
  // V30: 全域冷卻倒數計時器 (秒)
  const [apiCooldownSecs, setApiCooldownSecs] = useState(0);
  // V30: RPD 每日配額耗盡標誌 (到次日 UTC 00:00 為止)
  const [rpdExhaustedUntil, setRpdExhaustedUntil] = useState(0);
    ...
    if (currentAIModel === 'detecting') {
      ...
      if (apiKeys.length > 0) {
        KP.getAutoSelectedModel(apiKeys)
          .then((discovery) => {
            if (discovery?.modelId) setCurrentAIModel(discovery.modelId);
            else setCurrentAIModel('detectionFailed');
          })
    ...
    if (supabase && showCloudInspector && isCloudAuthed) {
      ...
      if (currentAIModel === 'detecting') {
        ...
        if (apiKeys.length > 0) {
           KP.getAutoSelectedModel(apiKeys)
             .then((discovery) => {
                console.log('[AI Discovery] 偵測完成，得到型號:', discovery?.modelId);
                if (discovery?.modelId) setCurrentAIModel(discovery.modelId);
                else setCurrentAIModel('detectionFailed');
             })
             .catch(err => {
  ...
  // V29.x: 智慧冷卻狀態，防止限流時持續撞牆
  const translationCooldownRef = useRef<number>(0);

  // V30: 每秒更新全域冷卻倒數
  useEffect(() => {
    const tick = setInterval(() => {
      const g = (globalThis as any)._TUC_COOLDOWN_UNTIL || 0;
      const local = translationCooldownRef.current || 0;
      const until = Math.max(g, local);
      setApiCooldownSecs(Math.max(0, Math.ceil((until - Date.now()) / 1000)));
      // 同步更新 RPD 耗盡狀態
      const rpdUntil = (globalThis as any)._TUC_RPD_EXHAUSTED_UNTIL || 0;
      setRpdExhaustedUntil(rpdUntil > Date.now() ? rpdUntil : 0);
    }, 1000);
    return () => clearInterval(tick);
  }, []);

  // V30: 強制清除所有冷卻狀態（使用者主動介入）
  const forceResetCooldown = () => {
    (globalThis as any)._TUC_COOLDOWN_UNTIL = 0;
    (globalThis as any)._TUC_RPD_EXHAUSTED_UNTIL = 0; // 同時清除 RPD 鎖定
    translationCooldownRef.current = 0;
    localStorage.setItem('_TUC_KEY_HEALTH', '{}');
    setApiCooldownSecs(0);
    setRpdExhaustedUntil(0);
    // 同時將所有 cooldown / error 欄位重設為 pending，讓它們下一輪自動重試
    setData(prev => {
      const newStatus = { ...(prev.bilingualStatus || {}) };
      Object.keys(newStatus).forEach(k => {
        if (newStatus[k] === 'cooldown' || newStatus[k] === 'error') {
          delete newStatus[k]; // 清除，讓下一輪 useEffect 重新掃描
        }
      });
      return { ...prev, bilingualStatus: newStatus };
    });
    console.log('[V30] 強制清除所有 API 冷卻狀態（包含 RPD 鎖定），重啟翻譯排程。');
  };
  ...
  const handleBilingualSync = async (force: boolean = false) => {
    ...
    const getStaticTranslation = (text: string, targetLang: Language): string | null => {
      ...
      // 2. 如果 text 是某個語系的翻譯文字，反查其 Key
      const foundKey = BOILERPLATE_KEYS.find(k => translations[srcLang][k] === text);
      if (foundKey) {
        return translations[targetLang][foundKey] || null;
      }
      return null;
    };
    ...
    textFields.forEach(k => {
      const val = (data[k] as string) || '';
      if (!val) return;
      
      // V521: 優先嘗試靜態字典查表
      const staticVal = getStaticTranslation(val, data.secondaryLanguage);
      if (staticVal) {
        // 直接存入快取，不進入 AI 隊列
        setData(prev => ({
          ...prev,
    ...
    data.tableData.forEach((row, rowIndex) => {
      ['category', 'item', 'spec', 'method', 'samples', 'confirmation'].forEach(col => {
        const key = `tableData_${rowIndex}_${col}`;
        const val = row[col as keyof typeof row] as string;
        if (!val || val === 'NA' || !/[\u0E00-\u0E7F]/.test(val)) return;
        
        const cachedSrc = data.bilingualCache[`${key}_src`];
        if (force || cachedSrc !== val) {
          payload[key] = val;
          needsSync.push(key);
        }
    ...
    if (Object.keys(payload).length === 0) {
      console.log('[AI Sync] 內容無變動，無需同步。');
      return;
    }

    // V30.4: 檢查全域冷卻狀態，避免在 API 熔斷時發起請求
    const cooldownUntil = (globalThis as any)._TUC_COOLDOWN_UNTIL || 0;
    if (Date.now() < cooldownUntil) {
      console.warn(`[AI Sync] 系統冷卻中，剩餘 ${Math.ceil((cooldownUntil - Date.now())/1000)} 秒，跳過同步。`);
      return;
    }

    setIsSyncingBilingual(true);
    // 標記狀態為 pending (翻譯中)
    setData(prev => {
      const newStatus = { ...(prev.bilingualStatus || {}) };
      needsSync.forEach(k => newStatus[k] = 'pending');
      return { ...prev, bilingualStatus: newStatus };
    });

    try {
      console.log(`[AI Sync] 啟動全量同步，目標語系: ${data.secondaryLanguage}，欄位數: ${needsSync.length}`);
      const results = await KP.translateFullBilingualState(payload, data.secondaryLanguage, apiKeys);
      
      // V30.4: 處理 Discovery 回傳 null (斷路器觸發) 的情況
      if (!results || results === payload) {
        console.warn('[AI Sync] 翻譯未執行（金鑰耗盡或模型過載）');
        setIsSyncingBilingual(false);
        return;
      }
      
      setData(prev => {
        const newCache = { ...prev.bilingualCache };
        const newStatus = { ...(prev.bilingualStatus || {}) };
        Object.keys(results).forEach(k => {
          newCache[k] = results[k];
          newCache[`${k}_src`] = payload[k]; // 記錄當前翻譯所依據的來源
          newStatus[k] = 'success';
        });
        return { ...prev, bilingualCache: newCache, bilingualStatus: newStatus };
      });
      console.log(`[AI Sync] 同步完成。消耗 1 次 API 配額。`);
    } catch (err: any) {
      console.error('[AI Sync] 同步失敗:', err);
      setData(prev => {
        const newStatus = { ...(prev.bilingualStatus || {}) };
        needsSync.forEach(k => newStatus[k] = 'error');
        return { ...prev, bilingualStatus: newStatus };
      });
    } finally {
      setIsSyncingBilingual(false);
  ...
  const forceBilingualTranslation = (fieldKey: keyof FormState) => {
    // 透過清空該欄位的 bilingual_src 狀態來強迫 useEffect 重新抓取並翻譯
    const newCache = { ...data.bilingualCache };
    delete newCache[`${fieldKey}_src`];
    
    const newStatus = { ...data.bilingualStatus };
    delete newStatus[fieldKey];

    setData(prev => ({
      ...prev,
      bilingualCache: newCache,
      bilingualStatus: newStatus
    }));
  };
            ...
            {(data.primaryLanguage !== data.secondaryLanguage || data.language !== data.secondaryLanguage) && (
              <button
                ...
                style={{
                  ...
                  alignItems: 'center',
                  gap: '6px',
                  background: isSyncingBilingual ? 'var(--bg-secondary)' : 'linear-gradient(135deg, #3B82F6 0%, #2563EB 100%)',
                  color: isSyncingBilingual ? '#888' : 'white',
                  border: 'none',
                  cursor: isSyncingBilingual ? 'not-allowed' : 'pointer',
                  boxShadow: isSyncingBilingual ? 'none' : '0 2px 6px rgba(37, 99, 235, 0.2)',
              ...
              >
                ...
                <span>{isSyncingBilingual ? t('syncingBilingual', data.language) : t('bilingualSync', data.language)}</span>
              </button>
            )}
          ...
          <div style={{
            ...
            {rpdExhaustedUntil > 0 ? (
              <span style={{
                ...
                🚫 {(() => {
                  const minsLeft = Math.ceil((rpdExhaustedUntil - Date.now()) / 60000);
                  const h = Math.floor(minsLeft / 60), m = minsLeft % 60;
                  return data.language === 'th-TH'
                    ? `โควต้ารายวัน (RPD) หมดแล้ว — รีเซ็ตใน ${h} ชม. ${m} น.`
                    : `今日配額 (RPD) 已耗盡 — ${h} 小時 ${m} 分後重置`;
                })()}
                <button
                  onClick={forceResetCooldown}
                  title={data.language === 'th-TH' ? 'บังคับรีเซ็ต (ไม่แนะนำ)' : '強制解鎖（不建議）'}
                  style={{
                    background: '#EF4444',
                    color: '#fff',
                    border: 'none',
                ...
                >
                  {data.language === 'th-TH' ? 'บังคับ' : '強制重啟'}
                </button>
              </span>
            ) : apiCooldownSecs > 0 && (
              <span style={{
                ...
                ⏳ {data.language === 'th-TH' ? `API พักระหว่างรอ... ${apiCooldownSecs} วิ` : `API 限流冷卻中... ${apiCooldownSecs} 秒`}
                <button
                  onClick={forceResetCooldown}
                  title={data.language === 'th-TH' ? 'รีเซ็ตการแปล' : '強制解除冷卻'}
                  style={{
                    background: '#F59E0B',
                    color: '#000',
                    border: 'none',
                ...
                >
                  {data.language === 'th-TH' ? 'รีเซ็ต' : '強制重啟'}
                </button>
              </span>
            )}
          </div>
        ...
        {(!isMobile || mobileAppTab === 'edit') && (
          <div style={{ minWidth: 0, height: '100%', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            {(() => {
              ...
              return <SpecForm data={data} onChange={setData} isSyncBlocked={isSyncBlocked} forceBilingualTranslation={forceBilingualTranslation} />;
            })()}
          </div>
        )}
