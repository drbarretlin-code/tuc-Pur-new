...
export async function getAutoSelectedModel(apiKeys: string | string[]): Promise<{ modelId: string; apiKey: string } | null> {
  ...
  const lastRpdDay = localStorage.getItem('_TUC_RPD_DAY') || '';
  if (lastRpdDay !== rpdResetKey) {
    (globalThis as any)._TUC_RPD_EXHAUSTED_SET = new Set<string>();
    (globalThis as any)._TUC_RPD_EXHAUSTED_UNTIL = 0;
    localStorage.setItem('_TUC_RPD_DAY', rpdResetKey);
    console.log(`[AI Discovery] 新的一天 (${rpdResetKey})，清空 RPD 追蹤紀錄。`);
  }

  let healthUpdated = false;
  Object.keys(keyHealth).forEach(k => {
    if (keyHealth[k] > now + 3600000) {
      delete keyHealth[k];
      healthUpdated = true;
    }
  });
  if (healthUpdated) localStorage.setItem('_TUC_KEY_HEALTH', JSON.stringify(keyHealth));
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    for (const mId of priorityList) {
      ...
      if (rpdExhaustedSet.has(`${keyId}_${mId}`)) {
        console.log(`[AI Discovery] 金鑰 ${keyId} + 模型 ${mId} 今日 RPD 已耗盡，跳過。`);
        continue;
      }
      ...
      try {
        ...
        // V30.4: 強化探針 - 使用實質翻譯測試而非 'hi'，防止 503 假性成功
        await Promise.race([
          model.generateContent({
            contents: [{ role: 'user', parts: [{ text: 'Translate "Technical Specification" to Traditional Chinese.' }] }],
        ...
        if (status === 503 || status === 500 || errMsg.includes('503') || errMsg.includes('500') || errMsg.includes('high demand')) {
          console.warn(`[AI Discovery] 模型 ${mId} 目前伺服器過載 (503/500)，暫停使用該模型。`);
          invalidModels.add(mId);
          continue; 
        }

        if (status === 429 || errMsg.includes('429') || errMsg.includes('quota') || errMsg.includes('exhausted')) {
          if (errMsg.includes('limit: 0')) {
            console.warn(`[AI Discovery] 模型 ${mId} 無免費額度，屏蔽該模型。`);
            invalidModels.add(mId);
            continue;
          }
          ...
          const isRPDExhaustion = errMsg.includes('PerDay') || errMsg.includes('PerDayPerProject');
          if (isRPDExhaustion) {
            const rpdKey = `${keyId}_${mId}`;
            rpdExhaustedSet.add(rpdKey);
            console.warn(`[AI Discovery] 金鑰 ${keyId} + 模型 ${mId} 的每日配額 (RPD) 已耗盡。`);
            continue;
          }

          keyHealth[keyId] = now + (delaySec * 1000);
          localStorage.setItem('_TUC_KEY_HEALTH', JSON.stringify(keyHealth));
          console.warn(`[AI Discovery] 金鑰索引 ${i} 限流 (429 RPM)，冷卻 ${delaySec} 秒。`);
          break; 
        }

        if (status === 404 || errMsg.includes('404')) {
          console.warn(`[AI Discovery] 模型 ${mId} 返回 404，屏蔽。`);
          invalidModels.add(mId);
          continue;
        }
        ...
        if (isRateLimited) {
          console.warn(`[AI Discovery] 金鑰索引 ${i} 遇到限流 (${status})，換 Key。`);
          if (!globalFallback) globalFallback = { modelId: mId, apiKey: currentKey };
          break; 
        }
        ...
        if (isKeyError) {
          console.warn(`[AI Discovery] 金鑰索引 ${i} 已失效，跳過。`);
          break;
        }

        console.warn(`[AI Discovery] 模型 ${mId} 偵測失敗 (${status || 'unknown'})，繼續嘗試下一個。`);
        continue;
      }
    }
  }

  // V30.4: 修復負數秒數 Bug，且在全數失效時不強行回傳
  if (keys.length > 0) {
    let minWait = Infinity;
    let soonestKey = null;
    
    for (const k of keys) {
      const kid = k.substring(0, 10);
      const health = keyHealth[kid] || 0;
      if (health > now) {
        const wait = health - now;
        if (wait < minWait) {
          minWait = wait;
          soonestKey = k;
        }
    ...
    if (soonestKey && minWait !== Infinity) {
      console.warn(`[AI Discovery] 所有可用組合皆在冷卻中。最快恢復剩餘: ${Math.ceil(minWait / 1000)} 秒。`);
      (globalThis as any)._TUC_COOLDOWN_UNTIL = now + minWait;
      return { modelId: priorityList[0], apiKey: soonestKey };
    }
  }

  if (globalFallback) return globalFallback;
  
  console.error(`[AI Discovery] 嚴重故障：金鑰池中所有組合皆無法連通或已耗盡額度。`);
  return null; 
}
...
Term 2 (req): ${reqK || ' '}
    ...
    console.warn('[AI Search] 搜尋字詞翻譯異常:', err);
    const errMsg = err.message || '';
    if (errMsg.includes('503') || errMsg.includes('500') || errMsg.includes('high demand')) {
      console.warn(`[AI Search] 該模型遭遇 503 伺服器過載，令目前快取模型失效。`);
      invalidateCachedModel();
    } else if (errMsg.includes('429') || errMsg.includes('Quota') || errMsg.includes('exhausted')) {
      const delayMatch = errMsg.match(/retry in (\d+(\.\d+)?)/);
      const delaySec = delayMatch ? parseFloat(delayMatch[1]) : 60;
      console.warn(`[AI] API 金鑰額度耗盡 (429)，啟動系統級保護冷卻期: ${delaySec} 秒`);
      (globalThis as any)._TUC_COOLDOWN_UNTIL = Date.now() + (delaySec * 1000);
      invalidateCachedModel();
    }
...
export async function translateHints(
  ...
  const prompt = `You are an expert technical translator. Translate the following array of technical requirement strings into ${targetLabel}.
  ...
  try {
    ...
    if (!Array.isArray(translatedTexts)) throw new Error('Invalid JSON array');
    ...
    const errMsg = err.message || '';
    if (retryCount < 2 && (errMsg.includes('503') || errMsg.includes('429') || errMsg.includes('Quota'))) {
      invalidateCachedModel();
      console.log(`[AI Translation] 遇到過載或限流，延遲 2 秒後重試 (${retryCount + 1}/2)...`);
      await new Promise(r => setTimeout(r, 2000));
      return translateHints(hints, targetLang, apiKey, retryCount + 1);
    }
...
export async function translateCloudMetadata(
  ...
  try {
    ...
    return items.map((item, idx) => {
      ...
      return translated ? { ...item, name: translated.name || translated.title, tags: translated.tags } : item;
    });
  } catch (err: any) {
    if (retryCount < 2 && (err.message?.includes('503') || err.message?.includes('429'))) {
      invalidateCachedModel();
      await new Promise(r => setTimeout(r, 2000));
      return translateCloudMetadata(items, targetLang, apiKey, retryCount + 1);
    }
    return items;
  }
}
...
/**
 * Waterfall V4: 專屬長文欄位翻譯
 */
export async function translateFormFields(
  ...
  try {
    ...
    return items.map(item => {
      const translated = translatedList.find((t: any) => t.id === item.id);
      return translated ? { id: item.id, translatedText: translated.translatedText } : { id: item.id, translatedText: item.text };
    });
  } catch (err: any) {
    if (retryCount < 2 && (err.message?.includes('503') || err.message?.includes('429'))) {
      invalidateCachedModel();
      await new Promise(r => setTimeout(r, 2000));
      return translateFormFields(items, targetLang, apiKey, retryCount + 1);
    }
    return items.map(i => ({ id: i.id, translatedText: i.text }));
  }
}
...
export async function translateFullSpec(
  ...
  try {
    ...
    if (retryCount < 1 && (err.message?.includes('503') || err.message?.includes('429'))) {
      invalidateCachedModel();
      await new Promise(r => setTimeout(r, 3000));
      return translateFullSpec(data, targetLang, apiKey, retryCount + 1);
    }
    return data;
  }
}
...
export async function translateFullBilingualState(
  ...
  try {
    ...
    if (retryCount < 2 && (err.message?.includes('503') || err.message?.includes('429'))) {
      invalidateCachedModel();
      await new Promise(r => setTimeout(r, 2000));
      return translateFullBilingualState(payload, targetLang, apiKeys, retryCount + 1);
    }
    return payload;
  }
}
