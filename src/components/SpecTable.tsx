        ...
        <tbody>
          {data.map((row, index) => (
            <tr key={index} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
              <td><input style={{ padding: '4px', width: '80px' }} value={(typeof row.category === 'string' && row.category.startsWith('default')) ? t(row.category, language) : row.category} onChange={(e) => updateCell(index, 'category', e.target.value)} placeholder={t('docTblCat', language)} /></td>
              <td><input style={{ padding: '4px' }} value={(typeof row.item === 'string' && row.item.startsWith('default')) ? t(row.item, language) : row.item} onChange={(e) => updateCell(index, 'item', e.target.value)} placeholder={t('docTblItem', language)} /></td>
