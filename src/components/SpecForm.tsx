...
// V30.5: 雲端載入已改為全量覆蓋模式，不再使用智慧合併與相似度演算法。

interface Props {
  data: FormState;
  onChange: (newData: FormState) => void;
  isSyncBlocked?: boolean;
  forceBilingualTranslation?: (fieldKey: keyof FormState) => void;
}
...
const SpecForm: React.FC<Props> = ({ data, onChange, isSyncBlocked = false, forceBilingualTranslation }) => {
  ...
  const loadHistoryHints = async (mode: number | 'all') => {
    ...
    const categoryMap: Record<number, {key: keyof FormState, regKey: keyof FormState, category: string, contextKeywords: string[]}[]> = {
      0: [
        ...
        { key: 'rangeHistoryHints', regKey: 'rangeRegHints', category: 'technical', contextKeywords: ['適用', '區間', '區域', 'Range', 'Area'] }
      ],
      1: [
    ...
    try {
      ...
      targets.forEach((t: any) => {
        transStatus[t.key] = 'translating';
        transStatus[t.regKey] = 'translating';
      });
      onChange({ ...data, searchStatus: transStatus });

      // V29.x: 智慧快取共用 - 如果背景翻譯已經產出了中文，直接使用，省下一整次 AI 呼叫
      const cachedZhName = data.bilingualCache['equipmentName'];
      const cachedZhDesc = data.bilingualCache['requirementDesc'];
      const currentEq = data.equipmentName;
      const currentDesc = data.requirementDesc;
      
      let variants: any = null;

      // 檢查快取是否有效（來源文字必須匹配）
      const isCacheValid = 
        cachedZhName && data.bilingualCache['equipmentName_src'] === currentEq &&
        cachedZhDesc && data.bilingualCache['requirementDesc_src'] === currentDesc &&
        cachedZhName !== currentEq;

      if (isCacheValid) {
        console.log('[AI Search] 命中雙語快取，跳過 AI 翻譯步驟。');
        variants = {
          eqVariants: [cachedZhName],
          reqVariants: [cachedZhDesc]
        };
      } else {
        // 若無快取，才呼叫 AI
        variants = await Promise.race([
          KP.translateSearchQueries(currentEq, currentDesc, apiKeys),
          new Promise((_, reject) => setTimeout(() => reject(new Error('Translation timeout')), 15000))
        ]);
      }
      ...
      console.error('[AI Search] 搜尋流程中斷:', err);
      // V29.x: 發生錯誤時必須重置狀態，否則 UI 會永久卡在「翻譯中」
      const errorStatus = { ...data.searchStatus };
      targets.forEach((t: any) => {
        errorStatus[t.key] = 'ai_error';
        errorStatus[t.regKey] = 'ai_error';
      });
      onChange({ ...data, searchStatus: errorStatus });
    } finally {
      setIsAnalyzing(false);
    }
  };
        ...
        onSelect={async (rawImportedData: any) => {
          ...
          if (data.primaryLanguage !== data.secondaryLanguage) {
            Object.entries(rawImportedData).forEach(([key, value]) => {
              ...
              } else if (key === 'tableData' && Array.isArray(value)) {
                // 處理驗收表格的雙語快取
                value.forEach((row: any, i: number) => {
                  ['category', 'item', 'spec', 'method', 'samples', 'confirmation'].forEach(subKey => {
                    if (row[subKey]) {
                      bilingualCache[`tableData_${i}_${subKey}`] = formatValue(row[subKey]);
                    }
                  });
                });
              }
          ...
          const merged: FormState = {
            ...INITIAL_FORM_STATE,
            // V30.5: 遵照使用者要求，雲端載入前先清空主編區所有預設文字（Boilerplate）
            // 確保「以被套用的內容為主」，不與舊有預設內容混合
            equipmentScope: '',
            rangeRange: '',
            envRequirements: '',
            regRequirements: '',
