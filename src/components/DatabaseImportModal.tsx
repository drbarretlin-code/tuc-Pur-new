    ...
    try {
      ...
      const mappedDocs = (data || [])
        ...
        .map(item => ({
          ...
          originalEquipmentName: item.display_name || item.original_name, // V521: 記錄原始名稱供雙語對照
              ...
              >
                <div className="db-item-info">
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <h4 style={{ margin: 0 }}>{doc.equipmentName}</h4>
                    {language === 'th-TH' && doc.originalEquipmentName && doc.originalEquipmentName !== doc.equipmentName && (
                      <span style={{ fontSize: '0.75rem', color: '#888', marginTop: '2px', borderLeft: '2px solid rgba(255,255,255,0.1)', paddingLeft: '6px' }}>
                        {doc.originalEquipmentName}
                      </span>
                    )}
                  </div>
                  <p>
