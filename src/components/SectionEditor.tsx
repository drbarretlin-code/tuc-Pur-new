...
interface Props {
  ...
  bilingualStatus?: 'pending' | 'success' | 'error' | 'cooldown' | 'typing';
  cachedSrc?: string;
          ...
          <div style={{ marginLeft: '8px', display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.65rem' }}>
            {bilingualStatus === 'pending' ? (
              <span style={{ color: '#60A5FA', display: 'flex', alignItems: 'center', gap: '2px' }}>
                <Loader2 size={10} className="animate-spin" /> {t('aiTranslatingStatus', language)}
              </span>
            ) : (cachedSrc && cachedSrc !== safeValue) ? (
              <span style={{ color: '#F59E0B', opacity: 0.8 }}>● {t('modifiedSyncNeeded', language)}</span>
            ) : bilingualStatus === 'success' ? (
